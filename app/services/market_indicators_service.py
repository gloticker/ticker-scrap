...
class MarketIndicatorsService:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(self.fear_greed_url, headers=headers) as response:
                    ...
                    current_value = fear_greed_data.get('score', 0)
                    rating = fear_greed_data.get('rating', 'Unknown')
                    return {
                        "Fear & Greed": {
                            "score": format_number(current_value),
                            "rating": rating
                        }
                    }
    ...
    async def publish_fear_greed_index(self):
        try:
            ...
            self.redis_client.publish(
                StreamChannel.INDEX.value, json.dumps(data))
    ...
    async def publish_btc_dominance(self):
        try:
            ...
            self.redis_client.publish(
                StreamChannel.CRYPTO.value, json.dumps(data))
